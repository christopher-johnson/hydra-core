...
hydra.httpClient.rawInvoke = function (headers, content, options) {
    var self = this;
    if (utils.iri(self.expects) === "http://www.w3.org/ns/hydra/core#IriTemplate") {
        var url = content;
    } else {
        url = self.link || self.document.iri;
    }
    return hydra.httpClient.request(self.method, url, headers, JSON.stringify(content), options);
};
...
hydra.httpClient.rawJsonLdInvoke = function (content, options) {
    ...
    return hydra.httpClient.rawInvoke.bind(self)(headers, content, options)
